...
app.controller('BankCardBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
	$scope.isBankCard = true;
    ...
    dropdownTemplateBankCardStatus = '<div>' +
        '<select ng-model="MODEL_COL_FIELD" ' +
        'ng-change="grid.appScope.updateEntity(col, row)" style="padding: 2px;">'
    for (var i = 0; i < Object.keys(objEnumInfo.BankCard.status).length; i++) {
        $scope.status.push({"id": i, "name": objEnumInfo.BankCard.status[i]})
        dropdownTemplateBankCardStatus += '<option ng-selected="MODEL_COL_FIELD==' + i
            + '" value=' + i + '>' + objEnumInfo.BankCard.status[i] + '</option>'
    }
    dropdownTemplateBankCardStatus += '</select></div>'
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'name', displayName: objFieldInfo.BankCard.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'bank', displayName: objFieldInfo.BankCard.bank, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'images', displayName: objFieldInfo.BankCard.images, width: 170, enableCellEdit: false, cellTemplate: uploadImageTemplateBankCard},
        {field: 'status', displayName: objFieldInfo.BankCard.status, width: 120, headerTooltip: '点击表头可进行排序, 通过直接下拉选取操作来更新数据', enableCellEdit: false, cellTemplate: dropdownTemplateBankCardStatus},
        {field: 'createdAt', displayName: objFieldInfo.BankCard.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'lastUpdateTime', displayName: objFieldInfo.BankCard.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.BankCard.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'refHostId', displayName: objFieldInfo.BankCard.refHostId, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/host?relate=id&relateValue={{COL_FIELD}}"><span ng-bind="COL_FIELD"></span></a>'},

        {field: 'host', displayName: objFieldInfo.BankCard.host, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/host?relate=id&relateValue={{COL_FIELD.id}}"><span ng-bind="COL_FIELD.name"></span></a>'},
    ];
    ...
    function refreshData(showMsg){
        var url = '/base/BankCard/all?page=' 
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/base/BankCard/' + content.id
    ...
    $scope.addContent = function(){
        ...
        createDialogService("/assets/html/edit_templates/bank_card.html",{
            id: 'editor',
            title: '新增',
            scope: $scope,
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/bankcard?startTime='
