var app = angular.module('FundBackRecordBackendApp', ['tm.pagination', 'ui.grid', 'ui.grid.resizeColumns', 'ui.grid.selection', 'ui.grid.edit', 'angularFileUpload', 'fundoo.services', 'simditor', 'ui.grid.autoFitColumns']);

var checkboxTemplateFundBackRecord = '<div><input type="checkbox" ng-model="MODEL_COL_FIELD" ng-click="grid.appScope.updateEntity(col, row)" /></div>';
...
app.controller('FundBackRecordBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
	$scope.isFundBackRecord = true;
    ...
    dropdownTemplateFundBackRecordStatus = '<div>' +
        '<select ng-model="MODEL_COL_FIELD" ' +
        'ng-change="grid.appScope.updateEntity(col, row)" style="padding: 2px;">'
    for (var i = 0; i < Object.keys(objEnumInfo.FundBackRecord.status).length; i++) {
        $scope.status.push({"id": i, "name": objEnumInfo.FundBackRecord.status[i]})
        dropdownTemplateFundBackRecordStatus += '<option ng-selected="MODEL_COL_FIELD==' + i
            + '" value=' + i + '>' + objEnumInfo.FundBackRecord.status[i] + '</option>'
    }
    dropdownTemplateFundBackRecordStatus += '</select></div>'
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'name', displayName: objFieldInfo.FundBackRecord.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'amount', displayName: objFieldInfo.FundBackRecord.amount, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'status', displayName: objFieldInfo.FundBackRecord.status, width: 120, headerTooltip: '点击表头可进行排序, 通过直接下拉选取操作来更新数据', enableCellEdit: false, cellTemplate: dropdownTemplateFundBackRecordStatus},
        {field: 'createdAt', displayName: objFieldInfo.FundBackRecord.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'lastUpdateTime', displayName: objFieldInfo.FundBackRecord.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.FundBackRecord.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'refHostId', displayName: objFieldInfo.FundBackRecord.refHostId, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/host?relate=id&relateValue={{COL_FIELD}}"><span ng-bind="COL_FIELD"></span></a>'},
        {field: 'refProductId', displayName: objFieldInfo.FundBackRecord.refProductId, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/product?relate=id&relateValue={{COL_FIELD}}"><span ng-bind="COL_FIELD"></span></a>'},

        {field: 'host', displayName: objFieldInfo.FundBackRecord.host, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/host?relate=id&relateValue={{COL_FIELD.id}}"><span ng-bind="COL_FIELD.name"></span></a>'},
        {field: 'product', displayName: objFieldInfo.FundBackRecord.product, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/product?relate=id&relateValue={{COL_FIELD.id}}"><span ng-bind="COL_FIELD.name"></span></a>'},
    ];
    ...
    function refreshData(showMsg){
        var url = '/base/FundBackRecord/all?page=' 
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/base/FundBackRecord/' + content.id
    ...
    $scope.addContent = function(){
        ...
        createDialogService("/assets/html/edit_templates/fund_back_record.html",{
            id: 'editor',
            title: '新增',
            scope: $scope,
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/fundbackrecord?startTime='
