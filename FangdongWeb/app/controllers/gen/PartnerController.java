...
public class PartnerController extends Controller implements IConst {
    ...
    public static Result partnerPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<Partner>> msg = BaseController.doGetAll("Partner",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(partner.render(msg.data));
        } else {
    ...
    public static Result partnerBackendPage() {
        return ok(partner_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_Partner")
    @Role("create_partner")
    public static Result add() {
        Msg<Partner> msg = new Msg<>();

        Form<PartnerParser> httpForm = form(PartnerParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            PartnerParser formObj = httpForm.get();            
            Partner newObj = new Partner();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("Partner", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Partner", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            }

            newObj.name = formObj.name;
            newObj.area = formObj.area;
            ...
            newObj.levelEnum = formObj.levelEnum;
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_Partner")
    @Role("update_partner")
    public static Result update(long id) {
        Msg<Partner> msg = new Msg<>();

        Partner found = Partner.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("Partner", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Partner", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            ...
            try{
                ...
                found.levelEnum = formObj.levelEnum;
    ...
    public static class PartnerParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(Partner.class, "name") + "不能为空";
            }
	...
	public static Result report(String startTime, String endTime) {
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(Partner.class) + "报表");
		// 设置列
        sheet2.setColumnWidth(0, 4000);
        sheet2.setDefaultColumnStyle(0, cellStyle);//name
        sheet2.setColumnWidth(1, 4000);
        sheet2.setDefaultColumnStyle(1, cellStyle);//area
        sheet2.setColumnWidth(2, 4000);
        sheet2.setDefaultColumnStyle(2, cellStyle);//phone
        sheet2.setColumnWidth(3, 4000);
        sheet2.setDefaultColumnStyle(3, cellStyle);//address
        sheet2.setColumnWidth(4, 4000);
        sheet2.setDefaultColumnStyle(4, cellStyle);//password
        ...
        sheet2.setDefaultColumnStyle(8, cellStyle);//credit
        sheet2.setColumnWidth(9, 4000);
        sheet2.setDefaultColumnStyle(9, cellStyle);//level_enum
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(Partner.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(Partner.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(Partner.class, "area"));//area
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(Partner.class, "phone"));//phone
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(Partner.class, "address"));//address
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(Partner.class, "password"));//password
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(Partner.class, "createdAt"));//created_at
        titleRow.createCell(6).setCellValue(TableInfoReader.getFieldComment(Partner.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(7).setCellValue(TableInfoReader.getFieldComment(Partner.class, "status"));//status
        titleRow.createCell(8).setCellValue(TableInfoReader.getFieldComment(Partner.class, "credit"));//credit
        titleRow.createCell(9).setCellValue(TableInfoReader.getFieldComment(Partner.class, "levelEnum"));//level_enum
        titleRow.createCell(10).setCellValue(TableInfoReader.getFieldComment(Partner.class, "comment"));//comment
		...
		for (int i = 0; i < list.size(); i++) {
            ...
            if (item.address == null) {
                ...
                cell3.setCellValue(item.address);
            }
            HSSFCell cell4 = row.createCell(4);
            if (item.password == null) {
                cell4.setCellValue("");
            } else {
                cell4.setCellValue(item.password);
            }
            ...
            cell7.setCellValue(EnumInfoReader.getEnumName(Partner.class, "status", item.status));
            HSSFCell cell8 = row.createCell(8);
            cell8.setCellValue(item.credit);
            HSSFCell cell9 = row.createCell(9);
            cell9.setCellValue(EnumInfoReader.getEnumName(Partner.class, "levelEnum", item.levelEnum));
