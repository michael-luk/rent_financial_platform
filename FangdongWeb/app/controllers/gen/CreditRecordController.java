...
public class CreditRecordController extends Controller implements IConst {
    
    public static Result creditRecordPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<CreditRecord>> msg = BaseController.doGetAll("CreditRecord",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(credit_record.render(msg.data));
        } else {
    ...
    public static Result creditRecordBackendPage() {
        return ok(credit_record_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_CreditRecord")
    @Role("create_credit_record")
    public static Result add() {
        Msg<CreditRecord> msg = new Msg<>();

        Form<CreditRecordParser> httpForm = form(CreditRecordParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            CreditRecordParser formObj = httpForm.get();            
            CreditRecord newObj = new CreditRecord();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("CreditRecord", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("CreditRecord", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            }

            newObj.name = formObj.name;
            newObj.creditRaise = formObj.creditRaise;
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_CreditRecord")
    @Role("update_credit_record")
    public static Result update(long id) {
        Msg<CreditRecord> msg = new Msg<>();

        CreditRecord found = CreditRecord.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("CreditRecord", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("CreditRecord", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            ...
            Transaction txn = Ebean.beginTransaction();
            try{
                found = CreditRecord.find.byId(id);
                            
                found.name = formObj.name;
                found.creditRaise = formObj.creditRaise;
    ...
    public static class CreditRecordParser {
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_CreditRecord")
    @Role("report_credit_record")
	public static Result report(String startTime, String endTime) {
		String fileName = TableInfoReader.getTableComment(CreditRecord.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";

		// 创建工作薄对象
		HSSFWorkbook workbook2007 = new HSSFWorkbook();
		// 数据
        
        Query<CreditRecord> query = Ebean.find(CreditRecord.class);
        if (StrUtil.isNotNull(startTime) && StrUtil.isNotNull(endTime)){
            query.where().between("createdAt", startTime, endTime);
        }
        query.orderBy("id desc");
		List<CreditRecord> list = query.findList();
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(CreditRecord.class) + "报表");
		// 设置列
        sheet2.setColumnWidth(0, 4000);
        sheet2.setDefaultColumnStyle(0, cellStyle);//name
        sheet2.setColumnWidth(1, 4000);
        sheet2.setDefaultColumnStyle(1, cellStyle);//credit_raise
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(CreditRecord.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(CreditRecord.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(CreditRecord.class, "creditRaise"));//credit_raise
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(CreditRecord.class, "status"));//status
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(CreditRecord.class, "createdAt"));//created_at
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(CreditRecord.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(CreditRecord.class, "comment"));//comment
        titleRow.createCell(6).setCellValue(TableInfoReader.getFieldComment(CreditRecord.class, "host"));//host_id
		...
		for (int i = 0; i < list.size(); i++) {
            ...
            cell1.setCellValue(item.creditRaise);
            HSSFCell cell2 = row.createCell(2);
            cell2.setCellValue(EnumInfoReader.getEnumName(CreditRecord.class, "status", item.status));
