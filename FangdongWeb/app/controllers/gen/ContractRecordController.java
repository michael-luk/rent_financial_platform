...
public class ContractRecordController extends Controller implements IConst {
    ...
    public static Result contractRecordPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<ContractRecord>> msg = BaseController.doGetAll("ContractRecord",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(contract_record.render(msg.data));
        } else {
    ...
    public static Result contractRecordBackendPage() {
        return ok(contract_record_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_ContractRecord")
    @Role("create_contract_record")
    public static Result add() {
        Msg<ContractRecord> msg = new Msg<>();

        Form<ContractRecordParser> httpForm = form(ContractRecordParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            ContractRecordParser formObj = httpForm.get();            
            ContractRecord newObj = new ContractRecord();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("ContractRecord", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("ContractRecord", uniqueFieldIssue)
                        + "]存在同名数据";
            ...
            newObj.contractTime = formObj.contractTime;
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_ContractRecord")
    @Role("update_contract_record")
    public static Result update(long id) {
        Msg<ContractRecord> msg = new Msg<>();

        ContractRecord found = ContractRecord.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("ContractRecord", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("ContractRecord", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            ...
            Transaction txn = Ebean.beginTransaction();
            try{
                found = ContractRecord.find.byId(id);
                            
                found.name = formObj.name;
                found.status = formObj.status;
                found.contractTime = formObj.contractTime;
    ...
    public static class ContractRecordParser {
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_ContractRecord")
    @Role("report_contract_record")
	public static Result report(String startTime, String endTime) {
		String fileName = TableInfoReader.getTableComment(ContractRecord.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";

		// 创建工作薄对象
		HSSFWorkbook workbook2007 = new HSSFWorkbook();
		// 数据
        
        Query<ContractRecord> query = Ebean.find(ContractRecord.class);
        if (StrUtil.isNotNull(startTime) && StrUtil.isNotNull(endTime)){
            query.where().between("createdAt", startTime, endTime);
        }
        query.orderBy("id desc");
		List<ContractRecord> list = query.findList();
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(ContractRecord.class) + "报表");
		// 设置列
        ...
        sheet2.setDefaultColumnStyle(2, cellStyle);//contract_time
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(ContractRecord.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(ContractRecord.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(ContractRecord.class, "status"));//status
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(ContractRecord.class, "contractTime"));//contract_time
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(ContractRecord.class, "createdAt"));//created_at
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(ContractRecord.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(ContractRecord.class, "comment"));//comment
        titleRow.createCell(6).setCellValue(TableInfoReader.getFieldComment(ContractRecord.class, "host"));//host_id
        titleRow.createCell(7).setCellValue(TableInfoReader.getFieldComment(ContractRecord.class, "product"));//product_id
		...
		for (int i = 0; i < list.size(); i++) {
			// 取出对象
			ContractRecord item = list.get(i);
			// 创建行
            ...
            cell1.setCellValue(EnumInfoReader.getEnumName(ContractRecord.class, "status", item.status));
            HSSFCell cell2 = row.createCell(2);
            cell2.setCellValue(DateUtil.Date2Str(item.contractTime));
