...
public class ProductRecordController extends Controller implements IConst {
    ...
    public static Result productRecordPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<ProductRecord>> msg = BaseController.doGetAll("ProductRecord",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(product_record.render(msg.data));
        } else {
    ...
    public static Result productRecordBackendPage() {
        return ok(product_record_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_ProductRecord")
    @Role("create_product_record")
    public static Result add() {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("ProductRecord", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("ProductRecord", uniqueFieldIssue)
                        + "]存在同名数据";
            ...
            newObj.fundBackDate = formObj.fundBackDate;
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_ProductRecord")
    @Role("update_product_record")
    public static Result update(long id) {
        Msg<ProductRecord> msg = new Msg<>();

        ProductRecord found = ProductRecord.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("ProductRecord", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("ProductRecord", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            ...
            Transaction txn = Ebean.beginTransaction();
            try{
                found = ProductRecord.find.byId(id);
                            
                found.name = formObj.name;
                found.inDate = formObj.inDate;
                found.outDate = formObj.outDate;
                found.fundBackDate = formObj.fundBackDate;
    ...
    public static class ProductRecordParser {
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_ProductRecord")
    @Role("report_product_record")
	public static Result report(String startTime, String endTime) {
		String fileName = TableInfoReader.getTableComment(ProductRecord.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";

		// 创建工作薄对象
		HSSFWorkbook workbook2007 = new HSSFWorkbook();
		// 数据
        
        Query<ProductRecord> query = Ebean.find(ProductRecord.class);
        if (StrUtil.isNotNull(startTime) && StrUtil.isNotNull(endTime)){
            query.where().between("createdAt", startTime, endTime);
        }
        query.orderBy("id desc");
		List<ProductRecord> list = query.findList();
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(ProductRecord.class) + "报表");
		// 设置列
        sheet2.setColumnWidth(0, 4000);
        sheet2.setDefaultColumnStyle(0, cellStyle);//name
        sheet2.setColumnWidth(1, 4000);
        sheet2.setDefaultColumnStyle(1, cellStyle);//in_date
        sheet2.setColumnWidth(2, 4000);
        sheet2.setDefaultColumnStyle(2, cellStyle);//out_date
        sheet2.setColumnWidth(3, 4000);
        sheet2.setDefaultColumnStyle(3, cellStyle);//fund_back_date
        ...
        sheet2.setDefaultColumnStyle(9, cellStyle);//host_id
        sheet2.setColumnWidth(10, 4000);
        sheet2.setDefaultColumnStyle(10, cellStyle);//product_id


		// 创建表头
		HSSFRow title = sheet2.createRow(0);
		title.setHeightInPoints(50);
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(ProductRecord.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(ProductRecord.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(ProductRecord.class, "inDate"));//in_date
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(ProductRecord.class, "outDate"));//out_date
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(ProductRecord.class, "fundBackDate"));//fund_back_date
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(ProductRecord.class, "amount"));//amount
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(ProductRecord.class, "status"));//status
        titleRow.createCell(6).setCellValue(TableInfoReader.getFieldComment(ProductRecord.class, "createdAt"));//created_at
        titleRow.createCell(7).setCellValue(TableInfoReader.getFieldComment(ProductRecord.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(8).setCellValue(TableInfoReader.getFieldComment(ProductRecord.class, "comment"));//comment
        titleRow.createCell(9).setCellValue(TableInfoReader.getFieldComment(ProductRecord.class, "host"));//host_id
        titleRow.createCell(10).setCellValue(TableInfoReader.getFieldComment(ProductRecord.class, "product"));//product_id
		...
		for (int i = 0; i < list.size(); i++) {
            ...
            cell1.setCellValue(DateUtil.Date2Str(item.inDate));
            HSSFCell cell2 = row.createCell(2);
            cell2.setCellValue(DateUtil.Date2Str(item.outDate));
            HSSFCell cell3 = row.createCell(3);
            cell3.setCellValue(DateUtil.Date2Str(item.fundBackDate));
            HSSFCell cell4 = row.createCell(4);
            cell4.setCellValue(item.amount);
            HSSFCell cell5 = row.createCell(5);
            cell5.setCellValue(EnumInfoReader.getEnumName(ProductRecord.class, "status", item.status));
            ...
            if (item.host == null) {
                ...
                cell9.setCellValue(item.host.name);
            }
            HSSFCell cell10 = row.createCell(10);
            if (item.product == null) {
                cell10.setCellValue("");
            } else {
                cell10.setCellValue(item.product.name);
            }
		}
