...
public class RentRecordController extends Controller implements IConst {
    
    public static Result rentRecordPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<RentRecord>> msg = BaseController.doGetAll("RentRecord",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(rent_record.render(msg.data));
        } else {
    ...
    public static Result rentRecordBackendPage() {
        return ok(rent_record_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_RentRecord")
    @Role("create_rent_record")
    public static Result add() {
        Msg<RentRecord> msg = new Msg<>();

        Form<RentRecordParser> httpForm = form(RentRecordParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            RentRecordParser formObj = httpForm.get();            
            RentRecord newObj = new RentRecord();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("RentRecord", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("RentRecord", uniqueFieldIssue)
                        + "]存在同名数据";
            ...
            newObj.payDate = formObj.payDate;
		    ...
		    Guest parentGuest = Guest.find.byId(formObj.refGuestId);
		    newObj.guest = parentGuest;
		    newObj.refGuestId = formObj.refGuestId;
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_RentRecord")
    @Role("update_rent_record")
    public static Result update(long id) {
        Msg<RentRecord> msg = new Msg<>();

        RentRecord found = RentRecord.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("RentRecord", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("RentRecord", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            ...
            try{
		        ...
		        Guest parentGuest = Guest.find.byId(formObj.refGuestId);
		        found.refGuestId = formObj.refGuestId;
		        found.guest = parentGuest;
    ...
    public static class RentRecordParser {
        ...
        public String validate() {
            ...
            if (Guest.find.byId(refGuestId) == null) {
                return "无法找到上级, 请重试.";
            }
            return null;
        }
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_RentRecord")
    @Role("report_rent_record")
	public static Result report(String startTime, String endTime) {
		String fileName = TableInfoReader.getTableComment(RentRecord.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";

		// 创建工作薄对象
		HSSFWorkbook workbook2007 = new HSSFWorkbook();
		// 数据
        
        Query<RentRecord> query = Ebean.find(RentRecord.class);
        if (StrUtil.isNotNull(startTime) && StrUtil.isNotNull(endTime)){
            query.where().between("createdAt", startTime, endTime);
        }
        query.orderBy("id desc");
		List<RentRecord> list = query.findList();
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(RentRecord.class) + "报表");
		// 设置列
        ...
        sheet2.setDefaultColumnStyle(5, cellStyle);//pay_date
        sheet2.setColumnWidth(6, 4000);
        sheet2.setDefaultColumnStyle(6, cellStyle);//comment
        sheet2.setColumnWidth(7, 4000);
        sheet2.setDefaultColumnStyle(7, cellStyle);//host_id
        sheet2.setColumnWidth(8, 4000);
        sheet2.setDefaultColumnStyle(8, cellStyle);//guest_id


		// 创建表头
		HSSFRow title = sheet2.createRow(0);
		title.setHeightInPoints(50);
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(RentRecord.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(RentRecord.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(RentRecord.class, "rent"));//rent
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(RentRecord.class, "createdAt"));//created_at
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(RentRecord.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(RentRecord.class, "status"));//status
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(RentRecord.class, "payDate"));//pay_date
        titleRow.createCell(6).setCellValue(TableInfoReader.getFieldComment(RentRecord.class, "comment"));//comment
        titleRow.createCell(7).setCellValue(TableInfoReader.getFieldComment(RentRecord.class, "host"));//host_id
        titleRow.createCell(8).setCellValue(TableInfoReader.getFieldComment(RentRecord.class, "guest"));//guest_id
		...
		for (int i = 0; i < list.size(); i++) {
            ...
            cell1.setCellValue(EnumInfoReader.getEnumName(RentRecord.class, "rent", item.rent));
            ...
            cell4.setCellValue(EnumInfoReader.getEnumName(RentRecord.class, "status", item.status));
            HSSFCell cell5 = row.createCell(5);
            cell5.setCellValue(DateUtil.Date2Str(item.payDate));
            ...
            if (item.host == null) {
                ...
                cell7.setCellValue(item.host.name);
            }
            HSSFCell cell8 = row.createCell(8);
            if (item.guest == null) {
                cell8.setCellValue("");
            } else {
                cell8.setCellValue(item.guest.name);
            }
		}
